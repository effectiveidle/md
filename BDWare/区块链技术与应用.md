...
1. 全网广播新的交易，通过合法性验证的被暂存，各节点维护候选区块
...
为了快速检测double spending，全节点要在内存中维护UTXO(Unspent Transaction Output)集合，表示还没有被花掉的交易的输出集合，该集合中的每个元素要给出产生这个输出的交易的哈希值以及它在这个交易里是第几个输出
...
消息传播采用flooding，收到消息后传播给邻居节点，重复的消息不传播
...
输入脚本给出赎回脚本和让赎回脚本正确运行所需要的的签名(数目不定)
...
- 将来希望改变：proof of work ---> proof of stake
...
![image-20210203161441552](../img/image-20210203161441552.png)
